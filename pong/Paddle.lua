...
function Paddle:move(keyup, keydown)
...
function Paddle:followBall(ball)
    ...
    self.prediction = ball.dy*(self.x-ball.x)/ball.x+ball.y
    if ball.dx <= 0 or (self.prediction-self.height/8 <= self.y and self.y <= self.prediction+self.height/8) then
       self.dy = 0
       return
    end
    if self.y > self.prediction then
        self.dy = -PADDLE_SPEED
    elseif self.y < self.prediction then
        self.dy = PADDLE_SPEED
    else
        self.dy = 0
    end
